...
- 仅支持对入站 WeChat 消息进行内联回复。Bridge 的 `push` 帧没有可由 OpenClaw `message` 工具指定的目标用户，因此**不支持主动发送消息**；插件会返回明确错误，不会伪造“已发送”回执。
- 配置中不提供 `allowFrom`；它无法在当前 Bridge 协议下提供可靠的访问保护。请在 Bridge/微信入口侧限制可访问用户。
- 支持文本 + 图片/视频/语音/文件：入站媒体会解码后落盘到会话 store 的 `media/` 目录，并以附件形式进入 OpenClaw；出站回复中的媒体（`mediaUrl`/`mediaUrls`）会转成 base64 通过 Bridge 回发微信。微信一次只能发送一个附件，多个附件时仅发送第一个并在正文中列出其余文件名。
